...
public class Pager {
	...
	private int pageSize=10; //페이지당 레코드수, mysql 에서의 limit문의 두번째 매개변수 값
	...
	private int startIndex;  //페이지당 mysql에서의 시작 index ,limit문의 첫째 매개변수 값
	private int num;// 페이지당 시작 번호
	
	
	public void init(int totalRecord, int currentPage) {
		//list.size() 를 사용하지 않느 이유? limit 를 이용하야 모든 레코드를 가져오지 않기 때문에
		...
		//thymeleaf break 문이 없으므로, lastpage에 대한 조건을 부여한다.
		//lastPage보다 totalPage가 더 작다면, totalPage가 lastPage가 되면 된다.
